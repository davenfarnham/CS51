...
let rec bit_manip (s: string) (size: int) (blst: int list) (l: int) m : (int list) =
  if l = String.length s then blst 
                                            ...
                                            if (((String.length value) + size) > 32) then (match bit_helper 0 value 0 0 with
							                                   | (num, space) -> bit_manip s space (num :: blst) (l + 1) m)
			                    else (match blst with
						  | [] -> raise Error
						  | hd :: tl -> let (num, space) = bit_helper hd value 0 size in
								 bit_manip s space (num :: tl) (l + 1) m)) 
	else raise Error)
;;
...
let _ = 
  let fs = [(45, ["a"]); (13, ["b"]); ( 12, ["c"]); (16, ["d"]); (9, ["e"]); (5, ["f"])] in
    let (encoding, decoding) = encode fs in
      let lst = bit_manip "abc" 0 [0] 0 encoding in
        print_list lst
