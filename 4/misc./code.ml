...
module LQueue = ListQueue;;

let _ = (LQueue.enqueue 3 (LQueue.enqueue 2 (LQueue.enqueue 1 LQueue.empty)))
...
struct
    ...
    let ( * ) i i' = i * i'
    let ( ~- ) i = (-1) * i 
    let to_string i = print_int i; print_string "\n"
    let to_int i = i
    let of_int i = i
end

(* create a new module of type Ints*)
module IA = Ints;;

(* print out addition *)
let _ = 
  let ia = IA.(one + one) in
    IA.to_string ia 

(* modules are useful in organizing the namespace. If you create a
 * signature (interface), which a module then implements, the implementations
 * of the functions are then sealed; I can only interact with the module
 * through the signature, which is public. 
 *) 

(* if you add to_int and of_int, it essentially lets you bypass the module.
 * For example, instead of IA.(one + one) you could write IA.of_int (1 + 1)
 * or (IA.of_int ((IA.to_int IA.(one + one)) * 3)) == 6.
 *)

assert (IA.to_int (IA.of_int 5) = 5) 
